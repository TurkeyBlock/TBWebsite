...
export async function getSupabaseGame(tableName:string, gameId:number):Promise<GetReturn>{
  ...
  try{
    const { data, error} = await createClient().functions.invoke(gameEdge, {
      method: 'PATCH',
      headers: {
...
export async function upsertSupabaseGame(functionMethod:"PATCH"|"POST", tableName:string, gameId:number|null, playKey:string|null, gameAction:string|null):Promise<UpsertGameReturn>{
  const { data: { session } } = await createClient().auth.getSession();
  const jwt = session?.access_token;
  const body = {
    table: tableName,
...
export async function upsertSupabaseGamePlayers(gameTable:string, gameId:number, playKey:string|null, action:string):Promise<UpsertPlayerReturn>{
  const { data: { session } } = await createClient().auth.getSession();
  const jwt = session?.access_token;
  ...
  try{
    const {data:{returnBody}, error} = await createClient().functions.invoke(playerEdge, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${jwt}`,
      },
    ...
    funcReturn.maxPlayers = returnBody.maxPlayers;
  } catch {
